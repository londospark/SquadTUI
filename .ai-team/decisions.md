...
**Why:** Simple, understandable architecture that allows parallel work — Linus can build screens against service interfaces, Rusty can implement services, Basher can test with mocks. Single version source in csproj prevents drift. Immutable records with value-equality and nullable references provide type safety. No early complexity — DI container added only when warranted.

**Consequences:** Linus, Rusty, and Basher can work independently against stable interfaces. Architecture is digestible in 5 minutes.
...
**Impacts:** Linus implements TabPanel, responsive breakpoints, colors, keyboard/mouse handlers. Danny may need AppState updates (SidebarWidth, NotificationQueue). Rusty feeds real data into dashboard components.
